...
head(marital.nz)
```
There are a total of four catagories for marital status `mstatus` and we will try to predict the probabilty of each based on the age. A multinomial regression model with a linear effect of `age` is fit to the data as
```{r fitvglm}
...
\end{aligned}
$$
where $\mathcal{M}(m,\boldsymbol{\beta})$ is the multinomial probability mass function with $m$ trials and probabilities $\boldsymbol{\pi} = (\pi_1,\pi_2,\pi_3)$. Using the linear predictor $\eta_{i,r} = \boldsymbol{x}_i^T\boldsymbol{\beta}_r$ and considering the logit model we have the probabilities:
\begin{equation}
